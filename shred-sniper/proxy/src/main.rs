...
struct CommonArgs {
    ...
    /// Path to the in-process sniper config (json). When set, pump.fun launches are
    /// detected and fired on inside the deshred thread, with no cross-process hop.
    #[arg(long, env)]
    sniper_config: Option<PathBuf>,

    /// Parse and reconstruct shreds even when no gRPC service is running. Implied by
    /// `--grpc-service-port` and by `--sniper-config`.
    #[arg(long, env, default_value_t = false)]
    reconstruct_shreds: bool,
...
Support: https://discord.com/invite/doublezerotech (#jito-shredstream)\n"
    ...
    let pump_create_sender = Arc::new(BroadcastSender::new(1_000));
    let fill_sender = Arc::new(BroadcastSender::new(1_000));

    // start the sniper before the forwarder: it fetches the pump.fun global account and
    // builds its transaction templates, and a misconfigured sniper must fail loudly here
    // rather than silently miss launches later
    let sniper = match args.sniper_config.as_ref() {
        Some(path) => match sniper::Sniper::start(path, exit.clone()) {
            Ok(s) => {
                info!("Sniper armed from {path:?}");
                Some(s)
            }
            Err(e) => {
                error!("Failed to start sniper: {e}");
                return Err(ShredstreamProxyError::SniperError(e));
            }
        },
        None => None,
    };
    let should_reconstruct_shreds =
        args.grpc_service_port.is_some() || args.reconstruct_shreds || sniper.is_some();
